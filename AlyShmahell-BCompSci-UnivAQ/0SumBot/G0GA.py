...
class individual(object):
	def __init__(self,size):
		...
		# proteins represent the final optimized paramters to be injected inside G0Bot
		self.proteins = []
	...
	def fitness(self, firstMap):
		...
		# injecting optimized parameters inside G0Bot
		replaceLine('./G0Bot/G0Bot.java', 'double oPWNumShipsProduction1 = ', '    double oPWNumShipsProduction1 = '+str(self.proteins[0])+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oPNumShips = ', '    int oPNumShips = '+str(int(self.proteins[1]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oPGrowthRate =  ', '    int oPGrowthRate =  '+str(int(self.proteins[2]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oENumShips = ', '    int oENumShips = '+str(int(self.proteins[3]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oEGrowthRate = ', '    int oEGrowthRate = '+str(int(self.proteins[4]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oNNumShips = ', '    int oNNumShips = '+str(int(self.proteins[5]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oNGrowthRate = ', '    int oNGrowthRate = '+str(int(self.proteins[6]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oDistance = ', '    int oDistance = '+str(int(self.proteins[7]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int fleetDivider = ', '    int fleetDivider = '+str(int(self.proteins[8]))+';')
		replaceLine('./G0Bot/G0Bot.java', 'int oLocalMinima = ', '    int oLocalMinima = '+str(int(self.proteins[9]))+';')
		...
		for i in range(firstMap,firstMap+5):
			...
			replaceLine('./fitOnot','java -jar PlanetWars-viz/PlayGame.jar', 'java -jar PlanetWars-viz/PlayGame.jar 2>&1 "maps/map'+str(i)+'.txt" 10000 10000 log.txt "java -jar 3rdPartyBots/Genebot.jar" "java -jar ./G0Bot/G0Bot.jar" 2>&1 >/dev/null | echo $(tail -1) > winner.txt')
			# calling fitOnot script
			evProc = subprocess.call("./fitOnot",shell=True)
			# finding the winner
...
class G0GA(object):
	...
	def genGen(self):
		...
		self.population.sort(key=lambda obj: obj.default_fitness)
