...
class neurencoder_base(object):
	...
	def build_net(self,
		...
		if net_name == 'alice':
			...
			std_dev       = tf.sqrt(var)            
			epsilon = tf.random_normal(tf.stack([tf.shape(net)[0], 8])) 
			z_value  = mean + tf.multiply(epsilon, tf.exp(std_dev))
			return z_value, mean, std_dev
		else:
			return net
...
class symmetric_training_model(object):
	...
	def build_training_model(self):
		...
		self.networks['bob'] = self.build_net(
		    'bob',
		    tf.concat([self.networks['alice'][0], self.placeholders['key_seed']],
		...
		latent_loss = -0.5 * tf.reduce_sum(1.0 + 2.0 * self.networks['alice'][2] - tf.square(self.networks['alice'][1]) - tf.exp(2.0 * self.networks['alice'][2]), 1)
		self.loss_functions = {
		    'bob': [
		        tf.reduce_mean(
		            tf.abs(self.placeholders['msg'] - self.networks['bob']) + latent_loss),
		        tf.reduce_mean(
		            tf.abs(self.placeholders['msg'] - self.networks['bob']) + latent_loss) +
		        (1. - tf.reduce_mean(
		            tf.abs(self.placeholders['msg'] - self.networks['eve'])))**
		    ...
		    'eve': [
		        tf.reduce_mean(
		            tf.abs(self.placeholders['msg'] - self.networks['eve'] + latent_loss))
		    ],
		    'alan': [
		        tf.reduce_mean(
		            tf.abs(self.placeholders['msg'] - self.networks['alan'] + latent_loss))
		    ]
		}
		...
		for net in self.training_variables:
			for var in self.training_variables[net]:
				logger.debug(var)
		''''''
		logger.debug('before optimizers')
		self.optimizers = {
		    'bob': [
		        tf.train.AdamOptimizer(
		            self.hyper_parameters["learning_rate"]).minimize(
