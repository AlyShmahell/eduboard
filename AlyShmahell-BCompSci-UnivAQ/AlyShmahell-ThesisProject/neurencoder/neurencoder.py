...
class model_builder(base):
	...
	def build_net(self, net_name, net_input, no_of_FC_layers, build_mode, halved_first_layer_flag=False):
		if build_mode == 'training':
			self.weights = [tf.get_variable(net_name+"_w"+str(whichFCL), 
						[(self.hyper_parameters["key_seed_len"],0)
							[halved_first_layer_flag==True and whichFCL==0]
							+self.hyper_parameters["msg_len"], 
							self.hyper_parameters["key_seed_len"]+self.hyper_parameters["msg_len"]],
							initializer=tf.contrib.layers.xavier_initializer()) 
							for whichFCL in range(self.hyper_parameters["No_of_FC_Layers"][net_name])]
		else:
			self.graph = tf.get_default_graph()
			self.weights = [self.graph.get_tensor_by_name(net_name+"_w"+str(whichFCL)+":0")
					for whichFCL in range(self.hyper_parameters["No_of_FC_Layers"][net_name])]
...
class result_processor(base):
	...
	def __init__(self):
		...
		self.model_data_subpath = './saved_model_data/' + datetime.datetime.now().strftime("%Y-%m-%d-%H:%M:%S")	
	...
	def save_model(self, build_mode):
		self.mode_data_relative_file_name = self.model_data_subpath + '/neurencoder-' + self.scheme + '-' + build_mode
		self.model_saver.save(self.tfsession, self.mode_data_relative_file_name + '-model')
...
class asymmetric_model_engine(result_processor, asymmetric_hyper_parameters, helper_functions,
					model_data, asymmetric_training_model, asymmetric_testing_model,
					model_trainer, model_builder, asymmetric_model_tester, general_model_engine, general_hyper_parameters):
	
	def __init__(self, tfsession, scheme):
		super().__init__()
		self.logger.info('%s class is initiated', 'asymmetric_model_engine')
		# self.logger.info('inherited classes stack:\n%s',np.array([x.__name__ for x in inspect.getmro(self.__class__)]))
		self.tfsession = tfsession
		self.scheme = scheme
		self.ignite_model()
		

class symmetric_model_engine(result_processor, symmetric_hyper_parameters, helper_functions,
					model_data, symmetric_training_model, symmetric_testing_model,
					model_trainer, model_builder, symmetric_model_tester, general_model_engine, general_hyper_parameters):
...
class hybrid_model_engine(result_processor, asymmetric_hyper_parameters, helper_functions,
